...
$(function() {
    ...
    /** データテーブルイベント登録 */
    $('#asset-datatable tbody').on('click', 'tr', MyPage.selectedRowHandler);
 ...
 */
MyPage.getAbrogates = function(data) {
    // 廃棄済一覧の取得
    var result = WNote.ajaxValidateSend('/api/asset/api-assets/search_abrogates', 'POST',　{
        'cond': data.cond
    });

    if (!result) {
        WNote.log(result);
        WNote.showErrorMessage('エラーが発生した為、廃棄済一覧が取得できませんでした。');
        return null;
    }
...
MyPage.showTable = function(data) {
    var result = MyPage.getAbrogates(data);
    if (result) {
        MyPage.datatable
            .clear()
