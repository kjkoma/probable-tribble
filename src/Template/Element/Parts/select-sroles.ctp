...
/**
 * システムロール（srolesのロールタイプがシステム）の選択BOX（Form）を表示する
 *  
 * - - -
...
$default  = isset($default)  ? $default  : $this->App->conf('WNote.DB.Sroles.Kname.sysgeneral');
$attr     = isset($attr)     ? $attr     : "";
$disabled_attr = ($disabled) ? 'disabled="disabled"' : '';
?>
<label class="select">
    <select name="<?= $name ?>" id="<?= $id ?>" class="input-sm <?= $class ?>"
            data-app-form="<?= $form ?>" data-app-form-default="<?= $this->AppGlobal->systemRoleId($default) ?>"
            <?= $attr ?> <?= $disabled_attr ?>>
        <?php foreach($this->AppGlobal->sroles() as $role) { ?>
