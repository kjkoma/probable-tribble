...
/**
 * 入庫区分の新規予定作成用の選択BOX（Form）を表示する
 *  
 * - - -
 * @param array   $instockKbn   名称マスタより取得した指定のリスト
...
<label class="select">
    <select name="<?= $name ?>" id="<?= $id ?>" class="input-sm <?= $class ?>"
            ...
            <?= $disabled_attr ?>>
        <?php if ($blank) { ?>
            <option value="">-- 入庫区分選択 --</option>
        <?php } ?>
        <?php foreach($instockKbn as $kbn) { ?>
            <?php if ($kbn['nid'] == $this->App->conf('WNote.DB.Instock.InstockKbn.new') ||
                      $kbn['nid'] == $this->App->conf('WNote.DB.Instock.InstockKbn.back')) { ?>
